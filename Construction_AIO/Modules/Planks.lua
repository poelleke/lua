...
local Data = require("Construction_AIO.Data.Data")
...
local function Banking()
    ...
    if Bank:IsPINOpen() then
        ...
        if not Config.UseBankPin then
            ...
            Planks.Stop("Bank PIN is required but disabled in the settings.")
            return

        end

        if Config.BankPin == nil or Config.BankPin == "" then

            Planks.Stop("No Bank PIN configured.")
            return

        end

        if Bank:EnterPIN(Config.BankPin) then
    ...
    if Config.LoadLastPreset then
        ...
        print("Preset failed. Switching to manual banking.")
    ...
    if not Bank:IsOpen() then
        ...
        if not UTILS.SleepUntil(function()
            ...
            --print("Open:", Bank:IsOpen(), "PIN:", Bank:IsPINOpen())
        ...
        -- Wait for next tick if PIN is open
        if Bank:IsPINOpen() then
            return
        end
    ...
    if not Bank:DepositInventory() then
        ...
        print("Failed to deposit inventory.")
        return
    ...
    if not UTILS.SleepUntil(function()
        ...
        print("Logs did not appear in inventory.")
        return
...
local function ClickSawmill()
    ...
    if not API.IsPlayerMoving_() and not API.isProcessing() then
        ...
        if not API.isProcessing() then
            print("Crafting didn't start. Stopping script.")
...
local function CleaningUp()
    ...
    if not API.isProcessing() then
        ...
        print("Script state: Cleaning Up.")
...
local function Checking()
    ...
    if Inventory:IsEmpty() then
        ...
        print("Inventory is empty. Banking.")
    ...
    if Inventory:InvItemcount(material.Log) >= 15 then
        ...
        print("Found selected logs. Going to sawmill.")
    ...
    -- No Logs / Wrong Logs
    ------------------------------------------------------------------------
    print("No selected logs found. Banking for safety.")
